import { Bell, Bot, Briefcase, Building2, ChevronDown, GraduationCap, Library, Menu, Moon, Search, ShieldCheck, Sun, X } from "lucide-react";
...
const resourceItems = [
  ...
  { label: "AI Career Assistant", path: "/ai-career-assistant", description: "Search, match, and prepare with real data", icon: Bot }
];
...
export default function AppLayout() {
  ...
  return (
    <div className="cyber-shell relative min-h-screen">
      ...
      <footer className="relative z-10 border-t border-white/10 px-4 py-8"><div className="mx-auto grid max-w-7xl gap-6 text-sm text-soft-gray lg:grid-cols-[1.4fr_2fr]"><p><strong className="text-white">CareerGrid</strong> connects learning, internships, jobs, startups, referrals, interview prep, and recruitment workflows in one professional platform.</p><div className="flex flex-wrap gap-4 lg:justify-end"><Link to="/startups">Startups</Link><Link to="/courses">Course</Link><Link to="/internships">Internship</Link><Link to="/jobs">Job</Link><Link to="/learning-platforms">Learning Platform</Link><Link to="/study-material">Study Material</Link><Link to="/certifications">Certification</Link><Link to="/ai-career-assistant">AI Assistant</Link><Link to="/admin/login">Admin</Link></div></div></footer>
    </div>
  );
}
